...
struct CollisionBodyWithLink {
  ...
  CollisionBodyWithLink(urdf::CollisionSharedPtr collision, urdf::LinkSharedPtr link, const size_t indexInCollisionArray,
...
class RobotBodyFilter : public filters::FilterBase<T> {
 public:
   ...
   */
  bool computeMask(const sensor_msgs::msg::PointCloud2 &projectedPointCloud, std::vector<RayCastingShapeMask::MaskValue> &mask,
  ...
  void createBodyVisualizationMsg(const std::map<point_containment_filter::ShapeHandle, const bodies::Body *> &bodies, const rclcpp::Time &stamp,
